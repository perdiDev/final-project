#!/usr/bin/env python3
"""Build a TensorRT engine with GPU-parallel EfficientNMS and on-the-fly ONNX Simplification.

The input ONNX file is never modified. The script runs ONNX Simplifier in-memory
to optimize the graph before passing it to TensorRT. By default the generated engine
is written next to it as ``<model>_efficientnms.engine``.
"""
...
def _load_network(trt: Any, logger: Any, model_path: Path, simplify: bool) -> Tuple[Any, Any, Any]:
    ...
    if simplify:
        try:
            ...
            raise RuntimeError(
                "Modul 'onnx' dan 'onnxsim' wajib dipasang untuk melakukan simplifikasi.\n"
                "Silakan jalankan di terminal: pip install onnx onnxsim"
            ) from exc
            
        print(f"Menjalankan ONNX Simplifier pada '{model_path}' secara in-memory (file asli aman)...")
        try:
            onnx_model = onnx.load(str(model_path))
            model_simp, check = onnxsim.simplify(onnx_model)
            if not check:
                raise RuntimeError("ONNX Simplifier gagal memvalidasi struktur model yang disederhanakan.")
            
            print("Simplifikasi berhasil. Mengurai graph ke dalam TensorRT...")
            success = parser.parse(model_simp.SerializeToString())
        except Exception as exc:
            raise RuntimeError(f"Terjadi kesalahan saat memproses ONNX Simplifier: {exc}") from exc
    else:
        print(f"Melewati ONNX Simplifier. Mengurai '{model_path}' langsung ke dalam TensorRT...")
...
def _inspect(args: argparse.Namespace) -> int:
    ...
    _, network, _ = _load_network(trt, logger, args.model, not args.no_simplify)
...
def _build(args: argparse.Namespace) -> int:
    ...
    builder, network, _ = _load_network(trt, logger, args.model, not args.no_simplify)
...
def _default_output(model: Path) -> Path:
    return model.with_name(f"{model.stem}_efficientnms_opt.engine")


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Tambahkan EfficientNMS_TRT GPU-parallel ke model ONNX dengan optimasi Simplifier (tanpa mengubah ONNX asli)."
    )
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Path engine keluaran (default: <model>_efficientnms_opt.engine).",
    )
    parser.add_argument(
        "--num-classes",
        type=int,
    ...
    parser.add_argument(
        "--no-simplify",
        action="store_true",
        help="Lewati langkah ONNX Simplifier (tidak disarankan jika ingin performa optimal).",
    )
    return parser
