...
int main(int argc, char *argv[]) {
    ...
    std::string config_path = "config/pgie_config.txt";
    ...
    for(int i=1; i<argc; i++) {
		...
		if (arg == "--config" && i + 1 < argc) {
		    ...
		    g_print("\nPenggunaan: %s [--config <path_to_config.txt>]\n", argv[0]);
		    g_print("Contoh: %s --config config/pgie_custom.txt\n\n", argv[0]);
	    }
    }

    g_print("==================================================\n");
    g_print("Menggunakan file konfigurasi YOLO: %s\n", config_path.c_str());
    g_print("==================================================\n");
