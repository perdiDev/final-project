...
// Visualisasi jejak (trail) tracker: garis penghubung posisi pusat objek pada
// beberapa frame terakhir, digambar di atas OSD murni sebagai alat bantu
// visual untuk melihat kontinuitas ID tracking. Tidak memengaruhi pipeline
// atau perilaku tracker itu sendiri.
constexpr std::size_t kTrailMaxPoints = 20;
constexpr guint64 kTrailMaxAgeFrames = 150;      // ~5 detik pada 30 FPS
constexpr guint64 kTrailPruneIntervalFrames = 90;
constexpr unsigned int kTrailLineWidth = 2;
...
private:
    ...
    struct TrackTrail {
        ...
        guint64 lastSeenFrame{0};
    };

    // Menghasilkan warna yang berbeda-beda tapi konsisten untuk tiap object_id,
    // supaya jejak (trail) tiap kendaraan mudah dibedakan dan identitas ID-nya
    // terlihat jelas tetap sama selama tracking berlangsung.
    static NvOSD_ColorParams colorForObjectId(guint64 objectId) {
        const double hue = std::fmod(static_cast<double>(objectId) * 137.508, 360.0);
    ...
    // Membuang entri jejak yang sudah lama tidak diperbarui (mis. objek benar-
    // benar keluar frame/hilang), supaya map tidak tumbuh tanpa batas selama
    // stream berjalan lama.
    void pruneStaleTrails() {
        for (auto it = objectTrails_.begin(); it != objectTrails_.end();) {
            if (osdFrameCounter_ - it->second.lastSeenFrame > kTrailMaxAgeFrames) {
                it = objectTrails_.erase(it);
            } else {
                ++it;
            }
        }
    ...
    GstPadProbeReturn processOsdBuffer(GstPadProbeInfo *info) {
        ...
        for (NvDsMetaList *frameNode = batchMeta->frame_meta_list; frameNode != nullptr;
            ...
            // Gambar garis jejak (trail) tracker: menghubungkan titik pusat
            // objek pada beberapa frame terakhir. Ini murni penanda visual di
            // OSD untuk memudahkan verifikasi bahwa tracker tetap menjaga ID
            // yang sama secara kontinu; tidak memengaruhi keputusan tracker.
            guint linesUsed = 0;
            for (NvDsMetaList *objNode = frameMeta->obj_meta_list; objNode != nullptr;
                ...
                const auto trailKey = std::make_pair(frameMeta->source_id, objMeta->object_id);
                TrackTrail &trail = objectTrails_[trailKey];
                trail.lastSeenFrame = osdFrameCounter_;

                const float cx = objMeta->rect_params.left + objMeta->rect_params.width * 0.5f;
                const float cy = objMeta->rect_params.top + objMeta->rect_params.height * 0.5f;
                trail.points.push_back({cx, cy});
                if (trail.points.size() > kTrailMaxPoints) {
                ...
                if (displayMeta == nullptr) {
                    break;
                }

                const NvOSD_ColorParams trailColor = colorForObjectId(objMeta->object_id);
                for (std::size_t i = 1; i < trail.points.size(); ++i) {
                    if (linesUsed >= MAX_ELEMENTS_IN_DISPLAY_META) {
                        displayMeta->num_lines = linesUsed;
                        nvds_add_display_meta_to_frame(frameMeta, displayMeta);
                        displayMeta = nvds_acquire_display_meta_from_pool(batchMeta);
                        linesUsed = 0;
                        if (displayMeta == nullptr) {
                            break;
                        }
                    }
                    NvOSD_LineParams &line = displayMeta->line_params[linesUsed++];
                    line.x1 = static_cast<unsigned int>(trail.points[i - 1].x);
                    line.y1 = static_cast<unsigned int>(trail.points[i - 1].y);
                    line.x2 = static_cast<unsigned int>(trail.points[i].x);
                    line.y2 = static_cast<unsigned int>(trail.points[i].y);
                    line.line_width = kTrailLineWidth;
                    line.line_color = trailColor;
                }
            }
        ...
        if (osdFrameCounter_ % kTrailPruneIntervalFrames == 0) {
            pruneStaleTrails();
        }
    ...
    std::map<std::pair<guint, guint64>, TrackTrail> objectTrails_;
    guint64 osdFrameCounter_{0};
