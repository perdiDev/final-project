...
rumusan masalah dan tujuan penelitian yang ditetapkan pada `BAB-1-Pendahuluan.md` §1.3–§1.4
sebagai berikut:
...
1. **Kinerja *real-time pipeline* Nvidia DeepStream pada Jetson Orin Nano (menjawab RM1/Tujuan
   ...
   *real-time* pada Jetson Orin Nano dengan margin yang besar terhadap ambang 30 FPS
   (`BAB-2-Metode-Penelitian.md` §2.6) — rata-rata FPS berkisar 51,52 (YOLOv9t) hingga 67,02
   ...
   *end-to-end*-nya (rata-rata 457,67 ms, rata-rata P95 536,32 ms) jauh di atas ketiga model lain,
   disebabkan *backpressure* pada `Lat_PreMux_ms` (360,92 ms) akibat interaksi arsitektur
...
2. **Pengaruh NMS paralel berbasis TensorRT *plugin* (EfficientNMS) terhadap efisiensi *pipeline*
   ...
   `Lat_Infer_ms` membaik tetapi hanya dalam orde sub-milidetik (§3.3.1), konsisten dengan biaya
...
Rekomendasi berkondisi yang disusun pada `BAB-3-Hasil-dan-Pembahasan.md` §3.6.1 merangkum hal
...
`BAB-3-Hasil-dan-Pembahasan.md` §3.6.2, berikut saran yang diajukan bagi penelitian lanjutan
...
3. **Mengeksplorasi jalur optimasi lain di luar *plugin* EfficientNMS itu sendiri**, mengingat
   temuan negatif pada RM2 (§3.3.2), mengikuti urutan yang direkomendasikan
...
5. **Menguji skenario tambahan** — kepadatan lalu lintas tinggi, kondisi cahaya rendah, cuaca
   ...
   (`04_benchmark_protocol.md` §4.3) — untuk menguji generalisasi temuan §3.2–§3.4 di luar satu
...
6. **Mengukur kualitas/ketahanan *tracking*** (jumlah *ID switch*, MOTA/IDF1) secara terpisah dari
   ...
   `BAB-1-Pendahuluan.md` §1.6 poin 5), agar gambaran *trade-off* NvDCF vs. NvSORT lebih
