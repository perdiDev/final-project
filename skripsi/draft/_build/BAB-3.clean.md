...
3. ***Tracking*: NvDCF vs. NvDCF_perf vs. NvSORT** (RM3) — membandingkan efisiensi komputasi
   tiga profil *tracker* DeepStream pada keenam konfigurasi model, **dibatasi pada aspek
...
|---|---|
| *Compute board* | NVIDIA Jetson Orin Nano (spesifikasi rinci varian SKU, jumlah CUDA core, kapasitas TOPS, dan mode daya yang dipakai akan dilengkapi merujuk pada *datasheet* resmi NVIDIA sesuai konfigurasi akhir perangkat pengujian). |
...
Versi persis GStreamer, GLib, dan CUDA Toolkit yang terpasang mengikuti *image* JetPack yang
digunakan pada perangkat pengujian (bukan dipin secara eksplisit oleh proyek ini, melainkan
memakai versi sistem via `pkg_check_modules` dan pencarian header CUDA otomatis); versi aktual
akan dicatat sebagai bagian dari metadata *reproducibility* penelitian.

### 3.2.3 Dataset

Dataset KITTI dengan pemetaan ulang 3 kelas kustom (`0=car`, `1=van`, `2=truck`). *Split*
validasi: **1.010 gambar, 4.722 instance** (car 4.167, van 405, truck 150) — identik untuk
seluruh model, sehingga hasil akurasi antar model dapat dibandingkan langsung. Ketimpangan
kelas (`van`/`truck` jauh lebih sedikit dari `car`) dicatat sebagai limitasi statistik saat
membahas hasil per-kelas.
...
Total 6 konfigurasi model di atas menjadi dasar 18 skenario RM3 (6 model × 3 *tracker*, §3.5
...
| NvDCF_perf | `config/tracker_nvdcf_perf.yml` | Preset resmi NVIDIA, tuning NvDCF pada sisi kecepatan |
...
- `scripts/run_all_benchmark.sh` — orkestrasi otomatis 18 skenario (6 model × 3 *tracker*)
...
6. **Pengukuran efisiensi komputasi *tracking* (RM3)** — jalankan
   `scripts/run_all_benchmark.sh` (18 skenario penuh: 6 model × 3 *tracker*), kumpulkan
...
Sesuai rekomendasi protokol benchmark, setiap skenario sebaiknya diulang 3–5 repetisi dengan
jeda 30–60 detik antar repetisi (stabilisasi termal), dan 10–15 detik awal tiap *run*
(*warm-up*) dibuang saat analisis. Jumlah repetisi aktual yang benar-benar dijalankan pada
perangkat pengujian akan dicatat pada Bab IV setelah pengujian selesai.

## 3.4 Arsitektur/Desain Pipeline

Pipeline penelitian ini mengalirkan data melalui tahapan berikut:

1. **Sumber input** — kamera ZED (stereo) atau berkas video, diteruskan ke `nvstreammux` untuk
   *batching* frame ke buffer NVMM.
2. **`nvstreammux`** meneruskan batch frame ke `nvinfer` (*Primary GIE*), tempat model YOLO
   yang diuji melakukan inferensi.
3. **`nvinfer`** meneruskan hasil deteksi ke `nvtracker`, yang menjalankan salah satu profil
   *tracker* yang dipilih melalui argumen `--tracker` (NvDCF, NvDCF_perf, atau NvSORT).
4. **`nvtracker`** meneruskan hasil ke `nvvideoconvert`, lalu ke `nvdsosd` yang menggambar
   *bounding box*, label kelas, dan overlay FPS pada frame.
5. **`nvdsosd`** meneruskan frame akhir ke *output sink* yang dapat berupa *stream* RTSP,
   monitor lokal (`nv3dsink`), atau berkas MP4.

Dua jalur pengukuran berjalan paralel terhadap alur utama di atas, keduanya melalui *pad probe*
yang tidak mengganggu jalur render kritis:

- Pada keluaran `nvdsosd`, sebuah *pad probe* mengirim data ke *Benchmark Logger Thread*
  (`GAsyncQueue`, thread terpisah) yang menulis metrik per-frame ke `fps.csv`.
- Pada keluaran `nvinfer` (opsional, diaktifkan lewat `--dump-detections`), sebuah *pad probe*
  mengirim data ke *Detection Dump Thread* yang menulis deteksi mentah ke
  `<model>_detections.jsonl`.

Secara independen dari pipeline GStreamer di atas, proses `tegrastats` berjalan konkuren untuk
merekam utilisasi hardware, yang kemudian diproses oleh komponen `LogParser` (C++) menjadi
...
3. **Efisiensi komputasi *tracking* (RM3)** — jalankan `scripts/run_all_benchmark.sh` untuk 18
   skenario (6 model × 3 konfigurasi *tracker*: NvDCF/NvDCF_perf/NvSORT). Bandingkan
...
4. **Verifikasi akurasi *as-deployed* FP16 (pendukung, bukan bagian dari rumusan masalah inti)**
   ...
   gambar val KITTI yang sama, dibandingkan terhadap proxy FP32, untuk memastikan tidak ada
   penurunan akurasi tersembunyi akibat kuantisasi/*parsing* custom.
   ...
   **Yang belum dilakukan**: (a) ekspor 1.010 gambar val + label dari notebook Kaggle ke
   ...
   yang diekspor **identik** dengan split yang menghasilkan angka Bab V (cek *seed*/`args.yaml`
   ...
   angka mAP dipercaya; (d) pembaruan dokumentasi hasil akurasi dengan angka aktual. Karena
   statusnya infrastruktur-siap-tapi-belum-dieksekusi, skenario ini ditulis di sini sebagai
   **rencana pengujian pendukung**, bukan hasil yang sudah ada — hasil aktualnya baru bisa
   masuk Bab IV setelah langkah (a)-(d) selesai.

## 3.6 Kriteria Evaluasi

| Sumbu | Metrik | Kriteria |
|---|---|---|
| RM1 (*baseline*) | FPS keseluruhan | *Real-time* didefinisikan relatif terhadap FPS sumber (kamera ZED, dikonfigurasi 30 FPS pada pengaturan benchmark; nilai final dikonfirmasi berdasarkan konfigurasi perangkat pengujian) |
